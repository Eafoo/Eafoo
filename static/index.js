...
function refreshGameLayer(box, loop, offset) {
    ...
    for (let j = 0; j < box.children.length; j++) {
        ...
        if (i == j) {
            ...
            r.notEmpty = true;
            i = (Math.floor(j / 4) + 1) * 4 + Math.floor(Math.random() * 1000) % 1 + 1;
        } else {
            r.notEmpty = false;
        }
    }
...
function shareText(score) {
    ...
    if (deviation_time > 23000) {
        return '倒计时多了' + ((deviation_time / 1000) - 20).toFixed(2) + "秒，本次成绩作废哦！";
    }
    SubmitResults();
    if (score <= 49) return '加油！我相信您可以的！！！';
    if (score <= 99) return '^_^加把劲，底力大王就是您！';
